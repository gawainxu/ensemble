...
def parse_option():
    ...
    parser.add_argument("--linear_model_path", type=str,
                        default="/save/SupCon/cifar10_models/cifar10_resnet18_original_data__vanilia__SimCLR_0.01_trail_0/last_linear.pth")
    parser.add_argument("--num_classes", type=int, default=6)
    parser.add_argument("--feat_dim", type=int, default=128)

    parser.add_argument("--exemplar_features_path", type=str,
                        default="/features/cifar10_resnet18_trail_0_128_0.05_256_train")
    ...
    parser.add_argument("--testing_unknown_features_path", type=str,
                        default="/features/cifar10_resnet18_trail_0_128_0.05_256_test_unknown")
...
def LoF(testing_features, sorted_exemplar_features, opt):
    ...
    for idx, testing_feature in enumerate(testing_features):
        ...
        # start LoF
        clf = LocalOutlierFactor(n_neighbors=opt.LoF_K, novelty=True, metric="cosine",
                                 contamination=opt.LoF_contamination)
...
def feature_classifier(opt):
    with open(opt.exemplar_features_path, "rb") as f:
        features_exemplar_backbone, _, labels_examplar = pickle.load(f)
    ...
    if opt.exemplar_features_path1 is not None:
        with open(opt.exemplar_features_path1, "rb") as f:  # !!!!!!!!
            features_exemplar_backbone1, _, labels_examplar1 = pickle.load(f)  #
            # _, features_exemplar_backbone, _, labels_examplar = pickle.load(f)         #
    ...
    if opt.exemplar_features_path2 is not None:
        with open(opt.exemplar_features_path2, "rb") as f:  # !!!!!!!!
            features_exemplar_backbone2, _, labels_examplar2 = pickle.load(f)  #
            # _, features_exemplar_backbone, _, labels_examplar = pickle.load(f)         #
    ...
    if opt.testing_known_features_path1 is not None:
        with open(opt.testing_known_features_path1, "rb") as f:  # !!!!!!!!
            features_testing_known_backbone1, _, labels_testing_known1 = pickle.load(f)  #
            # _, features_testing_known_backbone, _, labels_testing_known = pickle.load(f)           #
        ...
        features_testing_known_backbone = np.concatenate(
            (features_testing_known_backbone, features_testing_known_backbone1), axis=1)

    if opt.testing_known_features_path2 is not None:
        with open(opt.testing_known_features_path2, "rb") as f:  # !!!!!!!!
            features_testing_known_backbone2, _, labels_testing_known2 = pickle.load(f)  #
            # _, features_testing_known_backbone, _, labels_testing_known = pickle.load(f)           #
        ...
        features_testing_known_backbone = np.concatenate(
            (features_testing_known_backbone, features_testing_known_backbone2), axis=1)
    ...
    if opt.testing_unknown_features_path1 is not None:
        with open(opt.testing_unknown_features_path1, "rb") as f:  # !!!!!!!!
            features_testing_unknown_backbone1, _, labels_testing_unknown1 = pickle.load(f)  #
            # _, features_testing_unknown_backbone, _, labels_testing_unknown = pickle.load(f)            #
        ...
        features_testing_unknown_backbone = np.concatenate(
            (features_testing_unknown_backbone, features_testing_unknown_backbone1), axis=1)

    if opt.testing_unknown_features_path2 is not None:
        with open(opt.testing_unknown_features_path2, "rb") as f:  # !!!!!!!!
            features_testing_unknown_backbone2, _, labels_testing_unknown2 = pickle.load(f)  #
            # _, features_testing_unknown_backbone, _, labels_testing_unknown = pickle.load(f)            #
        ...
        features_testing_unknown_backbone = np.concatenate(
            (features_testing_unknown_backbone, features_testing_unknown_backbone2), axis=1)
